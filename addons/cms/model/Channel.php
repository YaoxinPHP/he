...
class Channel Extends Model
{
    ...
    public static function getNav($channel, $tag = [])
    {
        ...
        //修改
        if(isset($tag['flag'])){
            $res = Channel::where($tag['where'])->order('weigh desc,id desc')->select();
            return self::getStr($res,$tag['flag']);
        }
    ...
    public static function getStr($arr,$flag)
    {
        $str = '';
        foreach ($arr as $k => $v) {
           $str .= ($v['diyname'] == $flag)?"<a id='' class='cur' href='/cms/c/".$v['diyname']."'>".$v['name']."</a>":"<a id='' href='/cms/c/".$v['diyname']."'>".$v['name']."</a>";
        }
        return $str;
    }
}
